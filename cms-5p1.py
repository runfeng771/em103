...
def bump_next_login_run(minutes: int = 90):
...
def watchdog_job():
    ...
    try:
        ...
        if now_epoch > expected_epoch + grace_sec and last_start < expected_epoch - 1:
            ...
            log_sep("WATCHDOG")
            log("WARNING 检测到 90分钟自动登录可能未执行（MISFIRE），立即补跑一次登录 ...")
...
def start_scheduler():
    ...
    # 2) 每90分钟自动登录（next_run_time 初始按“现在+90分钟”）
    scheduler.add_job(
        scheduled_login_job,
        "interval",
        minutes=90,
        ...
        next_run_time=datetime.now(timezone.utc) + timedelta(minutes=90),
    )
    ...
    log_sep("SCHEDULER")
    log("INFO  自动登录任务已启动：每 90 分钟刷新一次 token（以最后一次成功登录时间为准对齐 next_run_time）")
